...
sociomatrix.to.nby3.edges <- function(sociomatrix, keep.labels=FALSE) {
  ...
  if (class(sociomatrix) != "matrix") {
    message ("sociomatrix.to.nby3.edges: Converting sociomatrix to matrix class.")
...
electrograph <- function(input, make.edgelist.symmetric=TRUE,
  ...
  #input=eies.nnb4; shortest.paths.get=TRUE; ohmic.properties.get=TRUE; make.edgelist.symmetric=TRUE; betweenness.get=FALSE; recourse.betweenness.get=FALSE;  fidelities=NULL; verbose=FALSE;  substitute.names=NULL; missing.numbers.imply.isolates=FALSE
  ...
  if (verbose) message("electrograph: edges processed.")
  ...
  #if (verbose) {print(nby3); print(node.ids); print(length(node.ids));}


  
  if (verbose) message("electrograph: processing network components.")
  ...
  if (verbose) message("electrograph: network components processed.")
